...
// generate the pattern by group 
            void FMCPUChecker::preSortbyLength(std::vector<std::string>& vec_pattern){
            ...
            int FMCPUChecker::initialize(const std::string &fileName)
            {
                ...
                // do pre-sort on pattern LIST
                 preSortbyLength(vec_pattern);

                return nerror;
            }

            bool FMCPUChecker::doFuzzyTask(int thread_id,
            ...
            bool FMCPUChecker::strFuzzy(const size_t upper_limit,
                                                const std::string &pattern,
                                                std::vector<std::vector<std::string>> &vec_grp_str)
            {
                for (unsigned i = 0; i < totalThreadNum; i++)
                {
                    worker[i] = std::async(std::launch::async, &FMCPUChecker::doFuzzyTask, this, i, upper_limit,
            ...
            bool FMCPUChecker::check(const std::string &t)
            {
        ...
        void FMChecker::preCalculateOffsetPerPU(std::vector<std::vector<std::string>> &vec_grp_str,
        ...
        int FMChecker::startFuzzyMatch(const std::string &xclbinPath, const unsigned int cardID)
        {
            // get devices
        ...
        int FMChecker::fuzzyMatchLoadVec(std::vector<std::string>& vec_pattern)
        {
            ...
            FMCPUChecker::preSortbyLength(vec_pattern);
        ...
        bool FMChecker::executefuzzyMatch(const std::string& t)
        {
