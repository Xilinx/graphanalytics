...
void inline PhaseLoop_MapHostBuff_prune(long NV,
    ...
    buff_host.weights = aligned_alloc<float>(NE_mem_1);
    buff_host.flag = aligned_alloc<ap_uint<8> >(NV);//opt
   ...
   // ap_uint<CSRWIDTHS>* axi_indicesdup = reinterpret_cast<ap_uint<CSRWIDTHS>*>(buff_host.indicesdup);
    ...
    //mext_in[16] = {(unsigned int)(4) | XCL_MEM_TOPOLOGY, axi_offsets, 0};
    //mext_in[17] = {(unsigned int)(0) | XCL_MEM_TOPOLOGY, axi_indices, 0}; //| (unsigned int)(29)
    //if (NE_mem_2 > 0) {
...
void inline PhaseLoop_MapClBuff_prune(long NV,
    ...
    buff_cl.db_flag = cl::Buffer(context, flag_RW, sizeof(ap_uint<8>) * (NV), &mext_in[18]);//assert(FLAGW == 8)
...
void PhaseLoop_UsingFPGA_1_KernelSetup_prune(bool isLargeEdge,
    ...
    //ob_in.push_back(buff_cl.db_offsets);//zyl
    //ob_in.push_back(buff_cl.db_indices);//zyl
...
double PhaseLoop_UsingFPGA_Prep_Init_buff_host_prune(int numColors,
    ...
    buff_host.config0[5] = NVl;
    //buff_host.config0[4] = 0;//zyl totItr? 0?

    buff_host.config1[0] = opts_C_thresh;
    buff_host.config1[1] = currMod;
    printf("maxv:%d, NEx2:%ld, vertexNum: %d, numColors:%d, edgeNum:%d, opts_C_thresh:%d, currMod:%d", (MAXNV), NEx2, vertexNum,numColors,edgeNum,opts_C_thresh,currMod);
...
void ConsumingOnePhase_prune(GLV* pglv_iter,
    ...
    eachTimeInitBuff = PhaseLoop_UsingFPGA_Prep_Init_buff_host_prune(
        pglv_iter->numColors, pglv_iter->NVl, pglv_iter->G, pglv_iter->M, opts_C_thresh, currMod, pglv_iter->colors, buff_host);
