...
///////////////////////////////////////////////////////////////////////
    ...
    for (int i = 0; i < repInt; i++) {
        similarity_kernel[i] = cl::Kernel(program, "denseSimilarityKernel4PU:{denseSimilarityKernel4PU_0}", &fail);
    }
    ...
    mext_o[4 * PUNUM] = {XCL_BANK0, sourceWeight, 0};
    mext_o[4 * PUNUM + 1] = {XCL_BANK0, sourceCoeffs, 0};

    for (int i = 0; i < repInt; i++) {
        mext_o[4 * PUNUM + 2 + i] = {XCL_BANK0, config[i], 0};
        mext_o[4 * PUNUM + 2 + repInt + i] = {XCL_BANK0, result_id[i], 0};
        mext_o[4 * PUNUM + 2 + 2 * repInt + i] = {XCL_BANK0, similarity[i], 0};
    }
...
#else
    denseSimilarityKernel4PU(config[0], sourceWeight, sourceCoeffs, weightDense[0], weightDense[1], weightDense[2],
...
///////////////////////////////////////////////////////////////////////
    ...
    std::vector<cl::Kernel> similarity_kernel(repInt);
    for (int i = 0; i < repInt; i++) {
        similarity_kernel[i] = cl::Kernel(program, "denseSimilarityKernel4PU:{denseSimilarityKernel4PU_1}", &fail);
    }
    std::cout << "INFO: kernel has been created" << std::endl;
    ...
    mext_o[4 * PUNUM] = {XCL_BANK4, sourceWeight, 0};
    mext_o[4 * PUNUM + 1] = {XCL_BANK4, sourceCoeffs, 0};

    for (int i = 0; i < repInt; i++) {
        mext_o[4 * PUNUM + 2 + i] = {XCL_BANK4, config[i], 0};
        mext_o[4 * PUNUM + 2 + repInt + i] = {XCL_BANK4, result_id[i], 0};
        mext_o[4 * PUNUM + 2 + 2 * repInt + i] = {XCL_BANK4, similarity[i], 0};
    }
