...
void networkSender(hls::stream<ap_uint<128> >& indexStreamIn,
...
void multiHashAggrCounter(ap_uint<32> byPass,
                          ...
                          ap_uint<96> overflowURAM[1 << aggrPU][2048]
#endif
...
template <int maxDevice, bool multiAggr>
...
#pragma HLS resource variable = offsetStreamEnd core = FIFO_SRL
    ...
    hls::stream<ap_uint<128> > netStream;
...
#pragma HLS resource variable = netStreamEnd core = FIFO_SRL
    ...
    loadHopMultipleIndex<!multiAggr>(numHop, intermediate, byPass, offsetStart, offsetEnd, offsetStream,
                                     offsetStreamEnd, index, localStream, localStreamEnd, netStream, netStreamEnd,
                                     aggrStream, aggrStreamEnd, outStream, outStreamEnd);
...
void readURAM1(ap_uint<96> overflowURAM[2048],
               hls::stream<ap_uint<128> >& aggrOutStream,
               hls::stream<bool>& aggrOutStreamEnd) {
#pragma HLS INLINE off
...
#pragma HLS INLINE off
    ...
    if ((byPass == 0) && (numResidual == 0)) {
        if ((numAggr >= aggrThreshold) || (batchEnd && (numAggr > 0))) {
            for (int i = 0; i < PU; i++) {
#pragma HLS UNROLL
...
#pragma HLS UNROLL
            ...
            if (overflowURAM[i][0] != 0) readURAM1(overflowURAM[i], aggrOutStream[i], aggrOutStreamEnd[i]);
        }
    }
}

template <int depth, int maxDevice, int hopPU, int aggrPU>
void nHopCore(bool loadBatch,
              ...
              ap_uint<64> cardTable[hopPU][maxDevice],
              ...
              ap_uint<96> overflowURAM[aggrPU][2048],
#endif
              ap_uint<32>& numAggr,
              ap_uint<32>& numResidual,
              ap_uint<512>* bufferPing,
              ap_uint<512>* bufferPong,
              hls::stream<ap_uint<128> > netStream[8],
...
#pragma HLS resource variable = aggrStreamEnd core = FIFO_SRL
    ...
    if (hopPU >= 1)
        hopProcessingUnit<maxDevice, multipleAggr>(
            numHop, intermediate, byPass, duplicate, switchStream[0], switchStreamEnd[0], offsetStart, offsetEnd,
            indexTable[0], cardTable[0], offset0, index0, indexStream[0], indexStreamEnd[0], netStream[0],
            netStreamEnd[0], aggrStream[0], aggrStreamEnd[0], outStream[0], outStreamEnd[0]);

    if (hopPU >= 2)
        hopProcessingUnit<maxDevice, multipleAggr>(
            numHop, intermediate, byPass, duplicate, switchStream[1], switchStreamEnd[1], offsetStart, offsetEnd,
            indexTable[1], cardTable[1], offset1, index1, indexStream[1], indexStreamEnd[1], netStream[1],
            netStreamEnd[1], aggrStream[1], aggrStreamEnd[1], outStream[1], outStreamEnd[1]);

    if (hopPU >= 4) {
        hopProcessingUnit<maxDevice, multipleAggr>(
            numHop, intermediate, byPass, duplicate, switchStream[2], switchStreamEnd[2], offsetStart, offsetEnd,
            indexTable[2], cardTable[2], offset2, index2, indexStream[2], indexStreamEnd[2], netStream[2],
            netStreamEnd[2], aggrStream[2], aggrStreamEnd[2], outStream[2], outStreamEnd[2]);

        hopProcessingUnit<maxDevice, multipleAggr>(
            numHop, intermediate, byPass, duplicate, switchStream[3], switchStreamEnd[3], offsetStart, offsetEnd,
            indexTable[3], cardTable[3], offset3, index3, indexStream[3], indexStreamEnd[3], netStream[3],
            netStreamEnd[3], aggrStream[3], aggrStreamEnd[3], outStream[3], outStreamEnd[3]);
    }
...
template <int maxDevice, int hopPU>
...
#pragma HLS loop_tripcount min = 2
        ...
        if (ping_pong_flag) {
            nHopCore<depth, maxDevice, hopPU, aggrPU>(
                loadBatch, batchEnd, numHop, intermediate, byPass, duplicate, hashSize, numPair, pair, pairEnd,
                offsetStart, offsetEnd, offsetTable, indexTable, cardTable, offset0, index0, offset1, index1, offset2,
                index2, offset3, index3, aggrURAM, overflowURAM, numAggr, numResidual, bufferPing, bufferPong,
                switchStream, switchStreamEnd, outStream, outStreamEnd);
        } else {
            nHopCore<depth, maxDevice, hopPU, aggrPU>(
                loadBatch, batchEnd, numHop, intermediate, byPass, duplicate, hashSize, numPair, pair, pairEnd,
                offsetStart, offsetEnd, offsetTable, indexTable, cardTable, offset0, index0, offset1, index1, offset2,
                index2, offset3, index3, aggrURAM, overflowURAM, numAggr, numResidual, bufferPong, bufferPing,
                switchStream, switchStreamEnd, outStream, outStreamEnd);
        }
        ...
        std::cout << "numAggr=" << numAggr << std::endl;
        std::cout << "accumAggr=" << accumAggr << std::endl;
        ...
        xf::graph::internal::Hop::outputURAM<depth, aggrPU>(batchEnd, byPass, hashSize, accumAggr, aggrThreshold,
                                                            numResidual, aggrURAM, overflowURAM, outStream,
                                                            outStreamEnd);
    }
...
template<int PU>
void dispatchNetID (hls::stream<ap_uint<128> >& strmIn,
        hls::stream<bool>& strmInEnd,
        hls::stream<ap_uint<128> > strmOut[PU],
        hls::stream<bool> strmOutEnd[PU]){
#pragma HLS INLINE off
...
#pragma HLS resource variable = outStreamEnd core = FIFO_SRL
    ...
    nHopPingPong<maxDevice, PU>(numHop, intermediate, batchSize, hashSize, aggrThreshold, byPass, duplicate, pair,
                                pairEnd, offsetTable, indexTable, cardTable, offset0, index0, offset1, index1, offset2,
                                index2, offset3, index3, bufferPing, bufferPong, switchStream, switchStreamEnd,
                                outStream, outStreamEnd);
#ifndef __SYNTHESIS__
    std::cout << "========================Ping-Pong End===================" << std::endl;
#endif

    hls::stream<ap_uint<512> > switchMergeStream("swtchMergeStream");
#pragma HLS stream variable = switchMergeStream depth = 512
#pragma HLS resource variable = switchMergeStream core = FIFO_BRAM
    hls::stream<bool> switchMergeStreamEnd("switchMergeStreamEnd");
...
#pragma HLS resource variable = outMergeStreamEnd core = FIFO_SRL
    ...
    merge128to512<false, true>(switchStream, switchStreamEnd, numSwitch, switchOut, switchOutEnd);
...
#pragma HLS INLINE off
    ...
    xf::graph::internal::Hop::nHopTop<maxDevice, numHopPU>(
        numHop, intermediate, batchSize, hashSize, aggrThreshold, byPass, duplicate, switchIn, switchInEnd, offsetTable,
        indexTable, cardTable, offset0, index0, offset1, index1, offset2, index2, offset3, index3, bufferPing,
        bufferPong, localBufferSize, switchBufferSize, bufferLocal, switchOut, switchOutEnd);
