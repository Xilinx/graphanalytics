...
void Handle::initOpLouvainModularity(const char* kernelName,
                                     char* xclbinFile,
                                     char* kernelAlias,
                                     unsigned int requestLoad,
                                     unsigned int deviceNeeded,
    ...
    oplouvainmod->init((char*)kernelName, xclbinFile, deviceID, cuID, requestLoad);
    oplouvainmod->initThread(xrm, kernelName, kernelAlias, requestLoad, deviceNeeded, cuPerBoard);
    delete[] cuID;
    delete[] deviceID;
};
    ...
    for (int i = 0; i < opNm; ++i) {
        if (strcmp(ops[i].operationName, "pagerank") == 0) {
            ...
            initOpConvertCsrCsc(ops[i].kernelName, ops[i].xclbinFile, ops[i].kernelAlias, ops[i].requestLoad,
                                ops[i].deviceNeeded, ops[i].cuPerBoard);
        } else if (strcmp(ops[i].operationName, "louvainModularity") == 0) {
            unsigned int boardNm = ops[i].deviceNeeded;
            if (deviceCounter + boardNm > numDevices) {
                std::cout << "Error: Need more devices" << std::endl;
                exit(1);
            ...
            deviceCounter += boardNm;
            initOpLouvainModularity(ops[i].kernelName, ops[i].xclbinFile, ops[i].kernelAlias, ops[i].requestLoad,
                                    ops[i].deviceNeeded, ops[i].cuPerBoard);
        } else {
...
//            for(int j=0; j<boardNm; j++)
//            	oplouvainmod->buff_hosts[j].freeMem();
//            delete[] oplouvainmod->buff_hosts;
        }
    }
    xrm->freeXRM();
};
