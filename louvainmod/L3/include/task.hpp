...
#ifdef __DEBUG__
		printf("DBG_TODO: t[i=%d] = q.getWork()  \n", i );
#endif
            t[i] = q.getWork();
#ifdef __DEBUG__
	printf("DBG_DONE: t[i=%d] = q.getWork()  \n", i );
#endif
        ...
        std::cout << "INFO: Allocated deviceID = " << deviceID << "\t cuID = " << cuID << "\t channelID = " << channelID
            ...
            unsigned int deviceID = resR[ID][0].deviceId ;
            unsigned int cuID = resR[ID][0].cuId;
            t[ID].execute(deviceID, cuID, channelID, xrm, resR[ID], instanceName);
