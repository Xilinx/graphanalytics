...
#ifdef PRINTINFO
#define PRINTINFO_LVPHASE
#endif
///////////////////////////////////////////////////////////////////////////////////////////////////////////
void PrintReport_MultiPhase(
		bool   opts_coloring,
		...
		)
{
    ...
    for(int i=0; i<phase; i++)
    	printf("+ %2.6f  ",  eachTimeInitBuff[i]);
    printf("\n");
    printf("Total time for Read buff_host  : %lf = \t", totTimeReadBuff);
    for(int i=0; i<phase; i++)
    	printf("+ %2.6f  ",  eachTimeReadBuff[i]);
    printf("\n");
    ...
    for(int i=0; i<phase; i++)
    	printf("+ %2.6f  ",  eachBuild[i]);
    printf("\n");
    printf("----- time for ToTeachSet      : %lf = \t", ToTeachSet);
    for(int i=0; i<phase; i++)
    	printf("+ %2.6f  ",  eachSet[i]);
    printf("\n");
...
void inline PhaseLoop_Kernel_Enable(
...
bool PhaseLoop_CommPostProcessing(
		long   NV,
		int    numThreads,
		double opts_threshold,
		bool   opts_coloring,
...
void inline PhaseLoop_MapHostBuff(
		long             NV,
		long             NE_mem_1,
		long             NE_mem_2,
...
void inline PhaseLoop_MapClBuff(
		long NV,
		long NE_mem_1,
		long NE_mem_2,
		std::vector<cl_mem_ext_ptr_t>& mext_in,
...
void PhaseLoop_UsingFPGA_1_KernelSetup(
		bool                    isLargeEdge,
		cl::Kernel              &kernel_louvain,
        std::vector<cl::Memory> &ob_in,
        std::vector<cl::Memory> &ob_out,
...
)
{
            ...
            if(isLargeEdge)
            	ob_in.push_back(buff_cl.db_indices2);
            ob_in.push_back(buff_cl.db_weights);
            if(isLargeEdge)
            	ob_in.push_back(buff_cl.db_weights2);
...
void PhaseLoop_UsingFPGA_2_DataWriteTo(
		cl::CommandQueue &q,
		std::vector<std::vector<cl::Event> > &kernel_evt0,
		std::vector<cl::Memory> &ob_in)
...
void PhaseLoop_UsingFPGA_3_KernelRun(
		cl::CommandQueue &q,
		std::vector<std::vector<cl::Event> > &kernel_evt0,
		std::vector<std::vector<cl::Event> > &kernel_evt1,
		cl::Kernel &kernel_louvain )
...
void PhaseLoop_UsingFPGA_4_DataReadBack(cl::CommandQueue &q, std::vector<std::vector<cl::Event> > &kernel_evt1,
		std::vector<cl::Memory> &ob_out )
{/* kernel_evt1 : migrate from dev to host*/
...
void PhaseLoop_UsingFPGA_Post_par(
		long           vertexNum,
		int            num_runsFPGA,
		KMemorys_host  &buff_host,
		struct timeval &tstartE2E,
        ...
        )
{
    ...
    PhaseLoop_UsingFPGA_Post(vertexNum, num_runsFPGA, buff_host, tstartE2E, tendE2E, kernel_evt1, C, totItr, currMod, totTimeE2E);
}


void inline PhaseLoop_MapHostBuff_prune(
		long             		NV,
		long             		NE_mem_1,
		long             		NE_mem_2,
		std::vector<cl_mem_ext_ptr_t>& mext_in,
...
void inline PhaseLoop_MapClBuff_prune(
		long NV,
		long NE_mem_1,
		long NE_mem_2,
		std::vector<cl_mem_ext_ptr_t>& mext_in,
		...
		KMemorys_clBuff_prune& 	buff_cl)
{
    ...
    if(NE_mem_2!=0)
    buff_cl.db_indicesdup2 = cl::Buffer(context, flag_RD, sizeof(int) * (NE_mem_2), &mext_in[20]);


}
void UsingFPGA_MapHostClBuff_prune(
		long             		NV,
		long             		NE_mem_1,
		long             		NE_mem_2,
		cl::Context      		&context,
...
void PhaseLoop_UsingFPGA_Post_par_prune(
		long           		vertexNum,
		int            		num_runsFPGA,
		KMemorys_host_prune  &buff_host,
		struct timeval 		&tstartE2E,
		...
		)
{
    ...
    std::cout << "INFO: Finish E2E execution" << std::endl;
    std::cout << "INFO: FPGA execution time of " << num_runsFPGA << " runs:" << exec_timeE2E << " us\n"
              << "INFO: Average execution per run: " << exec_timeE2E - exec_time0 * num_runsFPGA + exec_time0
              << " us\n"
              << "INFO: The iterations is: " << buff_host.config0[2] << "\n";
    std::cout << "-------------------------------------------------------" << std::endl;
}
...
#pragma omp parallel for
    ...
    while ( !isItrStop ) {
        ...
        if ( isUsingFPGA ) {
            PhaseLoop_UsingFPGA(NV, opts_C_thresh, opts_coloring,
        		    opts_threshold, numThreads, currMod, G, C, C_orig, totItr,
					buff_host, buff_cl, kernel_louvain, q,/*kernel's parameter*/
					num_runsFPGA, colors, totTimeE2E, totTimeColoring);
        } else {
            PhaseLoop_UsingCPU(
            		opts_threshold, numThreads, currMod, G, C, C_orig, totItr,
					nonColor, totTimeClustering);
        }
        /* General post-processing for both FPGA and CPU */
        isItrStop = PhaseLoop_CommPostProcessing(NV, numThreads, opts_threshold, opts_coloring, prevMod, currMod,
          		G, C, C_orig, nonColor, phase, totItr, numClusters,  totTimeBuildingPhase );
		...
		KMemorys_host_prune *buff_host)
{
    ...
    time1  = omp_get_wtime() - time1;
#ifdef PRINTINFO_LVPHASE
    printf("maxv:%ld, NEx2:%ld, vertexNum: %d, numColors:%d, edgeNum:%d, opts_C_thresh:%lf, currMod:%lf\n", (glb_MAXNV), NEx2, vertexNum,numColors,edgeNum,buff_host->config1[0],buff_host[0].config1[1]);
...
double PhaseLoop_UsingFPGA_Prep_Read_buff_host_prune_local(
		long           		vertexNum,
		KMemorys_host_prune &buff_host,
		int            		&eachItrs,
		//output
...
void PhaseLoop_UsingFPGA_Post_par_noRead(
		double*        p_eachTimeE2E,
		int            num_runsFPGA,
		int            num_iter,
		struct timeval &tstartE2E,
		struct timeval &tendE2E,
		...
		int    numThreads)
{
    ...
    std::cout << "\t\t UsingFPGA_MapHostClBuff Device Available: " << device.getInfo<CL_DEVICE_AVAILABLE>() << std::endl;
    ...
    while ( !isItrStop ) {
        ...
        if(hasGhost)
        	eachTimeReGraph[phase-1] = PhaseLoop_CommPostProcessing_par(pglv_orig, pglv_iter, numThreads, opts_threshold, opts_coloring,
        		 nonColor, phase, totItr, numClusters,  totTimeBuildingPhase ,
				      eachNum         [phase-1],
				      eachC           [phase-1],
				      eachM           [phase-1],
				      eachBuild       [phase-1],
        	...
        	eachTimeReGraph[phase-1] = PhaseLoop_CommPostProcessing(pglv_orig, pglv_iter, numThreads, opts_threshold, opts_coloring,
        		 nonColor, phase, totItr, numClusters,  totTimeBuildingPhase ,
				      eachNum         [phase-1],
				      eachC           [phase-1],
				      eachM           [phase-1],
    ...
    /* Print information*/
    PrintReport_MultiPhase(
    		opts_coloring, opts_minGraphSize, opts_threshold, opts_C_thresh, numThreads,
    		numClusters,
    	    totTimeClustering, totTimeBuildingPhase, totTimeColoring     ,
    	    prevMod, phase, totTimeE2E_2,  num_runsFPGA, totItr,
    	    eachTimeE2E_2, eachMod, eachItrs, eachClusters   );
    /* Print additional information*/
    PrintReport_MultiPhase_2(
    	    phase, totTimeE2E_2,
			totTimeAll, totTimeInitBuff,  totTimeReadBuff, totTimeReGraph, totTimeFeature,
    	    eachTimeInitBuff, eachTimeReadBuff, eachTimeReGraph, eachTimeE2E_2, eachTimePhase, eachTimeFeature,
    	    eachNum, eachC, eachM, eachBuild, eachSet);
    buff_host.freeMem();
    ...
    printf("TOTAL PrePre                   : %lf = %f(dev) + %lf(bin) + %lf(buff) +%lf\n" ,timePrePre ,timePrePre_dev ,timePrePre_xclbin ,timePrePre_buff ,timePrePre-timePrePre_dev-timePrePre_xclbin-timePrePre_buff);//eachTimePhase
    printf("TOTAL PostPost                 : %lf = %lf + %lf\n",timePostPost, timePostPost_feature, timePostPost - timePostPost_feature);//eachTimePhase
} // End of runM

void ConsumingOnePhase_prune(
		GLV*                   pglv_iter,
		double                 opts_C_thresh,
		KMemorys_clBuff_prune& buff_cl,
		KMemorys_host_prune&   buff_host,
...
GLV* LouvainGLV_general(bool hasGhost, int mode_flow, int id_dev, GLV* glv_src, char* xclbinPath, int numThreads, int& id_glv, long minGraphSize, double threshold, double C_threshold, bool isParallel, int numPhase){
	double time1 = omp_get_wtime();
	assert(glv_src);
	...
	if(mode_flow == MD_NORMAL){
		...
		runLouvainWithFPGA_demo_par_core(
				hasGhost,
				id_dev,
				glv_src,
				glv,
		...
		runLouvainWithFPGA_demo_par_core_prune(
				hasGhost,
				id_dev,
				glv_src,
				glv,
...
void LouvainGLV_general_batch_thread(bool hasGhost, int flowMode, int id_dev, int id_glv, int num_dev, int num_par, double* timeLv, GLV* par_src[], GLV* par_lved[], char* xclbinPath, int numThreads, long minGraphSize, double threshold, double C_threshold, bool isParallel, int numPhase)
{
	GLV* glv_t;
	...
	for(int p=id_dev; p<num_par; p+=num_dev){
		...
		glv_t = LouvainGLV_general(hasGhost, flowMode, id_dev, par_src[p],  xclbinPath,  numThreads, id_glv_dev, minGraphSize, threshold, C_threshold, isParallel, numPhase);
		par_lved[p] = glv_t;
		//pushList(glv_t);
