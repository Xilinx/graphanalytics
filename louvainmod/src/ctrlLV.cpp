...
TimeStt::TimeStt(char* nm){name =nm; time = omp_get_wtime();}
double TimeStt::End(){time = omp_get_wtime()-time; return time;};
void TimeStt::Print(){printf("TIME_STT: %s =%lf sec\n",name, time);};
...
int myCmd::cmd_GetCmd(){
	...
	{
		if( 0==strcmp(cmdlist[cmd_last], argv[0]))
			break;
	}
...
void CtrlLouvain::PrintMan(){
	printf("\033[1;37;40mWelcome to use this command line for running Louvain, the following commands can be used:\033[0m:: \n");
...
void CtrlLouvain::ShowPara(){
	...
	if(this->flowMode==MD_CLASSIC)
		printf("| \033[1;32;40m Golden Flow \033[0m \n");
	if(this->flowMode==MD_NORMAL)
		printf("| \033[1;32;40m Support partition \033[0m \n");
	if(this->flowMode==MD_FAST)
		printf("| \033[1;32;40m Support pruning for fast iteration \033[0m \n");
	printf("| \033[1;32;40m numDevice \033[0m  : %d\n", num_dev);
	//printf("coloring\t%d\n", coloring);    //= 1;    //default =1;
	//printf("numThreads\t%8x\t", numThreads);  //= 1;	//default =1;
	printf("|----[ Active Object ]---------------------------------------------------------------------------------------------------------------------------------------\n");
	printf("| \033[1;36;40m[glv_curr]\033[0m\t");if(glv_curr)glv_curr->printSimple();else printf("NULL\n");
	if(IsTempInLIst()){
	...
	printf("| \033[1;35;40m[ ParLV  ]\033[0m\t%s \t %s \t %s \t %s \t %s\n"
			...
			,parlv.st_Merged     ? "Merged"        : "Not Merged"
			,parlv.st_FinalLved  ? "st_FinalLved"  : "Not st_FinalLved"
					);
	printf("=============================================================================================================================================================\n");
}
...
#define CHECKCURR 	if(glv_curr==0){ printf("\033[1;31;40mERROR\033[0m: glv_curr should not be NULL for running Louvain\n"); return -1; }
int CtrlLouvain::pushList(GLV* p_push){
	list<GLV*>::iterator iter = glv_list.begin();//std::find(glv_list.begin(),glv_list.end(), p_push);
	while(iter!=glv_list.end()){
		if((*iter)->ID==p_push->ID)
			break;
		iter++;
	}
...
void GraphCat_general(graphNew* g_cat, graphNew* g1, graphNew* g2, long num_add){
	  ...
	  for(int i=0; i<g1->edgeListPtrs[nv1]; i++){
		  if(g1->edgeList[i].head > g1->edgeList[i].tail)
			  continue;
	  ...
	  for(int i=0; i<g2->edgeListPtrs[nv2]; i++){
		  if(g2->edgeList[i].head > g2->edgeList[i].tail)
			  continue;
...
void GraphCat_general(graphNew* g_cat, graphNew* g1, graphNew* g2){
	  ...
	  for(int i=0; i<nv1+1; i++)
	      g_cat->edgeListPtrs[i] = g1->edgeListPtrs[i];
	  for(int i=0; i<nv2+1; i++)
	      g_cat->edgeListPtrs[i+nv1] = g2->edgeListPtrs[i]+g1->edgeListPtrs[nv1];
	  for(int i=0; i<g1->edgeListPtrs[nv1]; i++){
...
//del id
	...
	double rate = 0;
	if(id_rate!=-1 && mycmd.argc >id_rate)
		rate = atof(mycmd.argv[id_rate+1]);
	this->SafeCleanTemp();
...
//save id
	...
	int id_id = mycmd.cmd_findPara("-id");
	if(id_id!=-1 && (mycmd.argc > (id_id+1) )){
		id=atoi( mycmd.argv[id_id+1]);
		p_glv = find(id);
	}

	int id_name = mycmd.cmd_findPara("-name");
	if(id_name!=-1 && (mycmd.argc > (id_name+1) )){
		strcpy( filename, mycmd.argv[id_name+1]);
		p_glv = glv_curr;
	}else{
		strcpy(filename,NameNoPath(p_glv->name));
		p_glv = glv_curr;
	}
...
{
	char nameBatch[1024];
	strcpy(nameBatch, fileName);//because cmd.argc will be overloaded which is source of fileName, copy it into nameBatch
...
int CtrlLouvain::exe_LV_LOADG(){
	...
	if( mycmd.cmd_findPara("-glv")!=-1){
		int idx =  mycmd.cmd_findPara("-glv");
		if(mycmd.argc<3){
			printf("\033[1;31;40mERROR\033[0m: mycmd.argc = %d, it should be '3' or more \n", mycmd.argc);
			return -1;
		}
		char* filename;
		if(idx==1)
			...
			filename=mycmd.argv[1];
		glv_curr = LoadGLVBin(filename, id_glv);
		this->glv_curr->SetName(filename);
...
int CtrlLouvain::man_LV_PG(){
	printf("Help \"pg\" used for printing graph content.\n");
...
int CtrlLouvain::exe_LV_UNIQ(){//1)cmd <start> <end>
	...
	}else
	{
		if(mycmd.argc==3){
			...
			if(p1<p2)
				stt.CountV(glv_curr->G, p1,p2);//, pedge, M_v );
...
void CtrlLouvain::print_glv_list(){
	...
	printf("=========================================================[ \033[1;35;40mLIST BEGIN\033[0m ]======================================================================================\n");
	int cnt=0;
	while(iter!=glv_list.end()){
	...
	printf("=========================================================[ \033[1;35;40mLIST   END\033[0m ]======================================================================================\n");
}
...
double CtrlLouvain::CtrlPar(GLV* src, int num_par, bool isPrun,  int th_prun){
	...
	parlv.Init(flowMode, src, num_par, this->num_dev);//should never release resource and object who pointed; Just work as a handle
...
int CtrlLouvain::exe_LV_PAR(){
	...
	if(idx_prun>0 && mycmd.argc>idx_prun){
		...
		if(th_prun >MAXGHOST_PRUN || th_prun<1 ){
			  printf("\033[1;31;40mERROR\033[0m: Wrong value of partition parameter for -prun which should be in range of 1 ~%d!\n", MAXGHOST_PRUN);
			  return -1;
		}
	}
	...
	if(idx_num>0 && mycmd.argc>idx_num){
		...
		if(num_par >MAX_PARTITION || num_par<1 ){
			  printf("\033[1;31;40mERROR\033[0m: Wrong value of partition parameter for -num which should be in range of 1 ~%d!\n", MAX_PARTITION);
			  return -1;
		}
	}
	...
	if(mycmd.argc>=3){
		if(!isNumber){
			...
			SafeCleanTemp();
			glv_temp = LouvainGLV_general_par(flowMode, glv_curr, num_par, num_dev,isPrun, th_prun,
					xclbinPath, numThreads,id_glv, minGraphSize, threshold, C_threshold, this->isParallel, this->numPhase);
		}
	}
	else if(mycmd.cmd_findPara("-lv")!=-1){
		if(parlv.st_ParLved == true){
			printf("\033[1;37;40mINFO\033[0m: Louvains for sub-graph have already been done!!!\n");
			return -1;
		}
	    if( CtrlPar_Louvain()==-1)
	    	return -1;
		...
		pushList(glv_temp);
			...
			glv_temp = parlv.FinalLouvain( xclbinPath, numThreads,id_glv, minGraphSize, threshold, C_threshold, this->isParallel, this->numPhase);
			parlv.timesPar.timeFinal = omp_get_wtime() - parlv.timesPar.timeFinal;
			parlv.UpdateTimeAll();
			parlv.PrintTime();
			pushList(glv_temp);
			//To update original graph with new number of community NC and modularity Q
...
int CtrlLouvain::exe_LV_LV(){
	...
	if(mycmd.cmd_findPara("-l3")!=-1){
#ifndef ONLY_FOR_L2
		if(this->p_handle0==NULL){
	...
	if(mycmd.cmd_findPara("-parallel")!=-1||mycmd.cmd_findPara("-p")!=-1)
		this->isParallel = true;
	if(mycmd.cmd_findPara("-noparallel")!=-1||mycmd.cmd_findPara("-nop")!=-1||mycmd.cmd_findPara("-noParallel")!=-1)
		this->isParallel = false;

	bool hasGhost=false;
	if(mycmd.cmd_findPara("-gh")!=-1||mycmd.cmd_findPara("-ghost")!=-1)
		hasGhost=true;

	if(mycmd.argc==1 || idx_phase!=-1){
		SafeCleanTemp();
		glv_temp = LouvainGLV_general(hasGhost, this->flowMode, 0, glv_curr, xclbinPath,  numThreads, id_glv, minGraphSize, threshold, C_threshold, this->isParallel, this->numPhase);
		return 0;
		...
		SafeCleanTemp();
		if(mycmd.cmd_findPara("-ll")!=-1)
			parlv.isMergeGhost=false;
		if(mycmd.cmd_findPara("-gh")!=-1)
			parlv.isMergeGhost=true;
		glv_temp = parlv.MergingPar2(id_glv);
		pushList(glv_temp);
	}
	return 0;
...
int CtrlLouvain::CtrlPar_Louvain_thread()
{
	if(parlv.num_par<=0 || parlv.st_Partitioned==false){
		 printf("\033[1;31;40mERROR\033[0m: Partitioned graphs are not set!\n");
		 return -1;
	}
	printf("\033[1;37;40mINFO\033[0m: Total %d sub-graphs will be mapped to %d devices\n",  parlv.num_par, this->num_dev);
	...
	//clock_t stime[this->num_dev], etime[this->num_dev];
	{
		for(int dev=0; dev< this->num_dev; dev++)
		{
			...
			bool hasGhost=true;
			td[dev] = std::thread(LouvainGLV_general_batch_thread,
					hasGhost,
					flowMode,
					dev, id_glv, this->num_dev, parlv.num_par, parlv.timesPar.timeLv, parlv.par_src, parlv.par_lved,
					xclbinPath,  numThreads,
		...
		for(int  dev=0; dev< this->num_dev; dev++) {
			...
			//std::cout<< "INFO: Thread " << dev << " need time is "<< (etime[d] - stime[d])/CLOCKS_PER_SEC << "s." << std::endl;
		 }
	}
#else
	{
		parlv.timeLv_dev[0] = LouvainGLV_general_batch(0, this->num_dev, parlv.num_par, parlv.timeLv, parlv.par_src, parlv.par_lved,
				xclbinPath,  numThreads, id_glv, minGraphSize, threshold, C_threshold, this->isParallel, this->numPhase);

		parlv.timeLv_dev[1] = LouvainGLV_general_batch(1, this->num_dev, parlv.num_par, parlv.timeLv, parlv.par_src, parlv.par_lved,
				xclbinPath,  numThreads, id_glv, minGraphSize, threshold, C_threshold, this->isParallel, this->numPhase);
	}
#endif
	for(int p=0; p<parlv.num_par; p++){
...
int CtrlLouvain::ParLV_thread(GLV* src, int num_par, bool isPrun,  int th_prun)
{
	...
	for(int dev=0; dev< this->num_dev; dev++)
	{
		...
		//parlv.timeLv_dev[dev] = LouvainGLV_general_batch(
				flowMode,
				src,
				dev, this->num_dev, parlv.num_par, parlv.timesPar.timeLv, parlv.par_src, parlv.par_lved,
				xclbinPath,  numThreads,
				minGraphSize, threshold, C_threshold, this->isParallel, this->numPhase);
	}
...
int CtrlLouvain::CtrlPar_PreMerge()
{
	if(parlv.num_par<=0 || parlv.st_ParLved==false){
		 printf("\033[1;31;40mERROR\033[0m: Partitioned graphs are not st_ParLved!\n");
		 return -1;
	}
	parlv.PreMerge();
	return 0;
...
int CtrlLouvain::exe_LV_DEV(){
    ...
    for(int d=0; d<num_dev; d++)
    {
		...
		std::cout << "\t\tDevice Max Compute Units    : " << devices[d].getInfo<CL_DEVICE_MAX_COMPUTE_UNITS>() << std::endl;
		std::cout << "\t\tDevice Global Memory        : " << devices[d].getInfo<CL_DEVICE_GLOBAL_MEM_SIZE>() << std::endl;
		std::cout << "\t\tDevice Max Clock Frequency  : " << devices[d].getInfo<CL_DEVICE_MAX_CLOCK_FREQUENCY>() << std::endl;
		std::cout << "\t\tDevice Max Memory Allocation: " << devices[d].getInfo<CL_DEVICE_MAX_MEM_ALLOC_SIZE>() << std::endl;
		std::cout << "\t\tDevice Local Memory         : " << devices[d].getInfo<CL_DEVICE_LOCAL_MEM_SIZE>() << std::endl;
...
int CtrlLouvain::ExeCmd(){
	...
	if(cmd==LVLOADG){
		...
		if(-1!=exe_LV_SETTH()){ShowPara();}
	}else if(cmd==LVSETTHC){
		if(-1!=exe_LV_SETTHC()){ShowPara();}
	}else if(cmd==LVPG){
		exe_LV_PG();
	}else if(cmd==LVRSTC){
		if(-1!=exe_LV_RSTC()){exe_LV_STATUS();}
	}else if(cmd==LVRSTCL){
		if(-1!=exe_LV_RSTCL()){exe_LV_STATUS();exe_LV_PL();}
	}else if(cmd==LVINFOG){
		exe_LV_INFOG();
	}else if(cmd==LVUNIQ){
		exe_LV_UNIQ();
	}else if(cmd==LVTEST){
		if(-1!=exe_LV_TEST()){ ShowPara(); exe_LV_PL();}
	}else if(cmd==LVPL){
		exe_LV_PL();
	}else if(cmd==LVPUSH){
		if(-1!=exe_LV_PUSH()){ ShowPara(); exe_LV_PL();}//ShowPara();
	}else if(cmd==LVPOP){
		if(-1!=exe_LV_POP()){ exe_LV_PL(); ShowPara();}
	}else if(cmd==LVDEL){
		if(-1!=exe_LV_DEL()){ exe_LV_PL(); ShowPara();}
	}else if(cmd==LVDEMO){
		exe_LV_DEMO();
	}else if(cmd==LV){
		if(-1!=exe_LV_LV()){ ShowPara();}
	}else if(cmd==LVPAR){
		if(-1!=exe_LV_PAR()){ ShowPara(); exe_LV_PL();}
	}else if(cmd==LVHISTORY){
		exe_LV_HISTORY();
	}else if(cmd==LVMAN){
		exe_LV_MAN();
	}else if( cmd==LVRENAME){
		if(-1!=exe_LV_RENAME()){ ShowPara(); exe_LV_PL();}
	}else if( cmd==LVMERGE){
		if(-1!=exe_LV_MERGE()){ ShowPara();}
	}else if( cmd==LVCAT){
		if(-1!=exe_LV_CAT()){ ShowPara();}
	}else if( cmd==LVSAVE){
		if(-1!=exe_LV_SAVE()){ ShowPara();}
	}else{
		;
	}
...
int CtrlLouvain::Run()
{
	...
	do{
		if(!glv_curr && !glv_temp && cnt_list==0){
			printf("\033[1;37;40mINFO\033[0m::glv_curr and glv_tmp is NULL; list is empty, please loadg <path> to load G from a file\n");
		}
...
int CtrlLouvain::Init(int mode, int numThreads, double opts_C_thresh, char* opts_xclbinPath, long opts_minGraphSize)
{
    ...
    this->C_threshold = opts_C_thresh;
    strcpy(this->xclbinPath, opts_xclbinPath);
    this->useKernel = true;
    this->exe_LV_DEV();
