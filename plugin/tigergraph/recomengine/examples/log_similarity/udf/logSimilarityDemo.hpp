...
inline void udf_load_glove(std::string glove_file)
{
    std::string line;
    // std::ifstream glovefile("glove.6B.50d.txt");
    std::ifstream glovefile(glove_file);
    logSimilarityDemo::get_glove_dict().clear();
    if(glovefile.is_open()) {
        while (std::getline (glovefile, line)) {
            std::stringstream ss(line);
            std::string word, num;
            std::array<int,50> vec;
            ss >> word;
            int count = 0;
            while (ss >> num){
                vec[count] = 100*std::stof(num); // CosineSim only supports int, use 2 digit precision
                count++;
            }
            logSimilarityDemo::get_glove_dict()[word] = vec;
            // cout << word ;
            // for (int i=0; i<50 ;++i)
            //     cout << " " << vec[i];
            // cout << endl;
        ...
        std::cout << "ERROR: Could not read glove file " << glove_file << std::endl;
    }
    // for (auto x : umap)
