...
int main(int argc, const char *argv[]) {
  ...
  const int splitNm =
      3; // kernel has 4 PUs, the input data should be splitted into 4 parts
  ...
  const int channelW =
      16; // AXI interface bitwidth is 512bits, which equals to 16 x 32bits

  int numVertices =
      10240 * 14 * cuNm * deviceNeeded; // total number of vertex read from file
  int numEdges = 200;                   // total number of edge read from file
  int32_t edgeAlign8 = ((numEdges + channelW - 1) / channelW) * channelW;
  int general =
      ((numVertices + deviceNeeded * cuNm * splitNm * channelsPU - 1) /
  ...
  int32_t **numVerticesPU =
      new int32_t *[deviceNeeded * cuNm]; // vertex numbers in each PU
  int32_t **numEdgesPU =
      new int32_t *[deviceNeeded * cuNm]; // edge numbers in each PU
  ...
  if (!parser.getCmdOption("-coeffs", tmpStr)) { // coeffs
    filenameCoeffs = "./data/coeffs.csr";
    std::cout << "INFO: coeffs file path is not set, use default "
              << filenameCoeffs << "\n";
  } else {
    filenameCoeffs = tmpStr;
    std::cout << "INFO: coeffss file path is " << filenameCoeffs << std::endl;
  }
  ...
  xf::graph::Graph<int32_t, int32_t> **g =
      new xf::graph::Graph<int32_t, int32_t> *[deviceNeeded * cuNm];
  int fpgaNodeNm = 0;
  for (int i = 0; i < deviceNeeded * cuNm; ++i) {
    g[i] = new xf::graph::Graph<int32_t, int32_t>("Dense", 4 * splitNm,
                                                  numEdges, numVerticesPU[i]);
    ...
    for (int j = 0; j < splitNm; ++j) {
      ...
      int depth =
          ((numVerticesPU[i][j] + channelsPU - 1) / channelsPU) * edgeAlign8;
  ...
  int32_t **sourceCoeffs = new int32_t *[requestNm];
  for (int i = 0; i < requestNm; ++i) {
    sourceWeight[i] = xf::graph::L3::aligned_alloc<int32_t>(
        sourceLen); // weights of source vertex's out members
    memset(sourceWeight[i], 0, sourceLen * sizeof(int32_t));

    sourceCoeffs[i] = xf::graph::L3::aligned_alloc<int32_t>(
        sourceLen); // weights of source vertex's out members
    memset(sourceCoeffs[i], 0, sourceLen * sizeof(int32_t));
  }
          ...
          for (int n = 0; n < numEdges; ++n) {
            ...
            if ((k == 0) && (i == 0) && (j == 0) && (l == 0)) {
              for (int m = 0; m < requestNm; ++m) {
                sourceWeight[m][n] =
                    g[k][0]
                        .weightsDense[i * channelsPU + j][l * edgeAlign8 + n];
              }
            }
          }
        }
  ...
  std::cout << "INFO: Load Coefficients" << std::endl;
  readInCoeffs<splitNm>(filenameCoeffs, sourceLen, requestNm, sourceCoeffs);

  //---------------- Load Graph -----------------------------------
  std::cout << "INFO: Load Graph" << std::endl;
  for (int i = 0; i < deviceNeeded * cuNm; ++i) {
    (handle0.opsimdense)
        ->loadGraphMultiCardNonBlocking(i / cuNm, i % cuNm, g[i][0]);
  }
  ...
  //---------------- Run L3 API -----------------------------------
  std::cout << "INFO: Run L3 API" << std::endl;
  for (int m = 0; m < requestNm; ++m) {
    eventQueue[m] = xf::graph::L3::cosineSimilaritySSDenseMultiCard(
        handle0, hwNm, sourceLen, sourceWeight[m], sourceCoeffs[m], topK, g,
  ...
  delete[] g;
  //---------------- Check Result ---------------------------------
  std::cout << "INFO: Check Result" << std::endl;
  int err = 0;
  for (int m = 0; m < requestNm; ++m) {
