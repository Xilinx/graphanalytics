...
void createHandleSimDense(class openXRM *xrm, clHandle &handle,
  ...
  std::cout << "DEBUG:" << __FUNCTION__ << " IDDevice=" << IDDevice << "="
            << devName << " CommandQueue=" << &handle.q << std::endl;

  std::cout << "DEBUG:" << __FUNCTION__ << " kernelName=" << kernelName
            << " kernelAlias=" << kernelAlias << std::endl;
  ...
  std::cout << "DEBUG: " << __FUNCTION__ << " instanceName0=" << instanceName0
            << " created" << std::endl;
...
void opSimilarityDense::bufferInitInt(clHandle *hds, std::string instanceName0,
  ...
  for (unsigned int i = 0; i < (unsigned int)(splitNm - 1);
       i++) { // calculate multi PU start address
    startID[i] = tmp;
...
int opSimilarityDense::computeInt(unsigned int deviceID, unsigned int cuID,
  ...
  std::cout << "DEBUG: " << __FUNCTION__ << " deviceID" << deviceID
            ...
            << " cuPerBoardSimDense=" << cuPerBoardSimDense << " handles index="
